...
const EmployeeProfile = () => {
  ...
  const {data} = useQuery({
    queryKey: ['getpending'],
    queryFn: async () => {
      const responses = await axiosInstance.get('/leave')
    ...
    <div className="min-h-screen bg-gray-50 px-4 py-6">
      <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-lg border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          ...
          <Link to={'/leave/history'} className="flex items-center gap-3 bg-blue-50 p-4 rounded-md border border-blue-200">
            <MdOutlinePendingActions className="h-6 w-6 text-blue-500" />
            <div>
              <p className="text-sm text-gray-500">Pending Leaves</p>
              <p className="text-lg font-semibold text-gray-800">{data?.leave.length || "0"}</p>
            </div>
          </Link>
          <Link to={'/leave/history'} className="flex items-center gap-3 bg-green-50 p-4 rounded-md border border-green-200">
